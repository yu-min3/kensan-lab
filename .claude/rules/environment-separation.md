...
|-------|-----------|-------|-----------------|
| Infrastructure | `istio-system`, `argocd`, `cert-manager`, `vault`, `vault-config-operator`, `external-secrets`, `sealed-secrets`, `reloader`, `monitoring`, `cilium-secrets`, `kube-system`, `longhorn-system`, `cloudflare-tunnel` | PE only | `platform-project` |
| Platform Services | `platform-auth-prod`, `auth-system`, `backstage` | PE only | `platform-project` |
| Application | `app-{name}`（ADR-006、新規。例: `app-kensan`）/ `app-prod`（既存共有 ns）/ `kensan`（移行前 ns） | AD（PE が ns bootstrap） | `app-project` |
...
|---------|-------|--------------------|
| `platform-project` | Infrastructure + platform services | `kube-system`, `istio-system`, `monitoring`, `argocd`, `vault`, `vault-config-operator`, `auth-system`, `cert-manager`, `external-secrets`, `sealed-secrets`, `reloader`, `cloudflare-tunnel`, `longhorn-system`, `platform-auth-prod`, `backstage`, etc. |
| `app-project` | App applications | `kensan`, `app-prod`, `app-kensan`（ADR-006 で順次拡張） |
...
vault, vault-config-operator, external-secrets, sealed-secrets, reloader,
cloudflare-tunnel, longhorn-system, app-prod, app-kensan
```
