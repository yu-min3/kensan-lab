...
SELECT
    'd1c40001-0000-0000-0000-000000000000'::uuid,
    'd1c00001-0000-0000-0000-000000000000'::uuid,
    3,
    ...
    'ai', 'pending',
    '{"interaction_count": 8, "avg_rating": 4.4, "strengths": ["React学習のコード例が実践的", "カジュアルで親しみやすいトーン"], "weaknesses": ["行動パターンデータを活用したスケジューリングが不十分", "感情面へのフォローが不足", "学習の継続性と目標達成への時間配分の言及が少ない"]}'::jsonb
FROM ai_contexts WHERE id = 'd1c00001-0000-0000-0000-000000000000'::uuid;

-- chat v2: カジュアルなコミュニケーション強化 (manual)
INSERT INTO ai_context_versions (
    id, context_id, version_number,
    system_prompt, allowed_tools, max_turns, temperature, changelog,
    source, candidate_status, eval_summary
...
SELECT
    ...
    allowed_tools, max_turns, temperature,
    'カジュアルトーン + パーソナライズ: スケジューリング、目標整合性、感情配慮',
    'manual', NULL, NULL
FROM ai_contexts WHERE id = 'd1c00001-0000-0000-0000-000000000000'::uuid;
...
UPDATE ai_contexts SET active_version = 2 WHERE id = 'd1c00001-0000-0000-0000-000000000000'::uuid;  -- chat (v2 is latest manual)
UPDATE ai_contexts SET active_version = 1 WHERE id = 'd1c00002-0000-0000-0000-000000000000'::uuid;  -- review
UPDATE ai_contexts SET active_version = 1 WHERE id = 'd1c00003-0000-0000-0000-000000000000'::uuid;  -- daily_advice
UPDATE ai_contexts SET active_version = 1 WHERE id = 'd1c00004-0000-0000-0000-000000000000'::uuid;  -- persona
