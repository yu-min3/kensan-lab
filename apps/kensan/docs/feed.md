...
`/feed`画面は最新レポートを次のzoneに、この順で表示します。

1. 30秒サマリー（レポートにある場合のみ）
2. Inbox: 要対応
3. Inbox: 確認
4. 今日のニュース
5. リリース・定点観測

既知zoneに無い見出しは、この後ろへそのまま並べます。**レポートの見出しは
repoの外（Claude Scheduled Taskのプロンプト）で決まるため、新しいzoneは
予告なく増えます。** 未知zoneを末尾へ流すのは、増えても壊れないようにする
ためですが、読む順が意味を持つものは既知zoneへ昇格させる必要があります
（`30秒サマリー`は最下部に描画されていたのを2026-08-11に先頭へ移しました）。

`validation.go`の必須見出しは`要対応` / `今日のニュース` / `リリース・定点観測`
の3つだけです。`確認`と`30秒サマリー`は必須にしていません — 必須にすると
プロンプト側の揺れでimportごと落ちるため、表示側で吸収する方針です。
