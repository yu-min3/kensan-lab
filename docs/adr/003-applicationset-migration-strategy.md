...
*(Drafted retroactively on 2026-06-07 by working backwards from the classification table below; review welcome.)*

A category moves to ApplicationSet when **all** of the following hold; otherwise it stays on individual Applications:

1. **Uniform source shape** — every app in the category has the same source structure (e.g. Helm chart + values ref), so one template covers all of them without per-app patches
2. **Additions are expected** — new components arrive often enough that "drop in a `config.json`" scaffolding pays for the generator's indirection
3. **No dense per-app uniqueness** — no sync-wave ordering, per-app `ignoreDifferences`, or mixed tooling (Helm/Kustomize) that would force complex `templatePatch` logic
4. **No external writer** — nothing else (e.g. Backstage auto-commit) writes Application CRs into the category; generated and hand-written CRs in one directory would be confusing
...
The classification table above no longer matches the implementation on two rows:

- **Environments** was ultimately implemented as **individual Applications**, not an ApplicationSet. After
  dev/prod separation was abolished, the category's contents shifted to per-namespace lifecycle apps with
  heterogeneous shapes (dedicated ns manifests, `directory.include` filters, whole-directory sources), which
  fails criterion 1 (uniform source shape). No `config.json` files exist under `kubernetes/environments/`.
- Two ApplicationSet-based capabilities were added later that this table predates:
  **vault-database-engine** and **vault-transit-engine** (self-made chart + per-instance `platform-values/`).

Observability remains the reference ApplicationSet implementation as designed. The current pattern usage is
documented in `kubernetes/argocd/applications/README.md`; this ADR's criteria (above) still describe the
decision logic accurately.
