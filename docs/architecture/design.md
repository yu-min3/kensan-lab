...
|-----------|---------------|---------------|-------------------|
| `goldship-platform` | クラスター基盤、セキュリティ、Argo CD制御構造（App Project、Root Apps） | PE | Istio、Keycloak、Sealed Secrets、Namespaces、RBAC、Argo CD Applications |
...
5. ADがBackstage経由で新規アプリを作成すると：
   ...
   - BackstageがApplication CRを`goldship-platform/base-infra/argocd/applications/`に自動コミット
...
- **自動GitOps**: BackstageがArgo CD Application CRを`goldship-platform`に自動コミット（実装予定）
