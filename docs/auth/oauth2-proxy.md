...
- Istio `meshConfig.extensionProviders[oauth2-proxy]` で `envoy_ext_authz_http` として登録 (`kubernetes/network/istio/istiod/values.yaml`)
...
`failOpen: false` (Istio 側設定) なので、oauth2-proxy が落ちると保護対象 host (Backstage / Hubble / Prometheus / Longhorn 等) は 503 を返す。Grafana は bypass (app-native auth) なので CUSTOM ext_authz の対象外で、oauth2-proxy 落下の影響を受けない (oauth2-proxy の Bearer を Grafana が API key と誤解釈して 403 になる問題を避けるため、`authorizationpolicy-gateway-platform-allow.yaml` の Category 1 で素通しにしている)。
