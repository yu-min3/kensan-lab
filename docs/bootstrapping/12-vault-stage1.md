...
- `kubeseal` CLI がインストール済み (Sealed Secrets controller は `infrastructure/secrets/sealed-secrets/` でデプロイ済み前提)
- ap-northeast-1 リージョンに `alias/vault-unseal-kensan` の KMS key を作成済み

## Vault chart 構成 (参考)

`infrastructure/secrets/vault/values.yaml` で定義済みの主要パラメータ:
...
kubeseal --format=yaml < temp/vault-aws-kms-credentials-raw.yaml \
  > infrastructure/secrets/vault/resources/aws-kms-credentials-sealed.yaml
```
