...
- **bare ns の label remediation** — `ns-label-contract` rule 1 の violation として `cpolr` に出る想定 (sync 後に実測してこのリストを更新する):
  - `reloader` / `sealed-secrets`: ns manifest の追加 or `managedNamespaceMetadata` で対応 (kubernetes/secrets/ 配下)
  - `backstage`: `CreateNamespace=true` の bare ns。`managedNamespaceMetadata` で remediate する場合は **既存 live label (`kensan-lab.platform/vault-managed-postgres` 等、CCNP selector が参照) を含めた full set** で慎重に — 欠落させると vault dynamic creds の NetworkPolicy が壊れる
  ...
  - ※ `kensan` ns は対象外 — `apps/kensan-legacy/manifests/namespace.yaml` (kensan app が recurse で deploy) に environment + tier 付与済み (実機 label でも確認済み)
