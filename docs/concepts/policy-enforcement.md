...
- **bare ns の label remediation** (`reloader` / `sealed-secrets` / `cilium-secrets`): `ns-label-contract` rule 1 の violation として `cpolr` に出る。reloader / sealed-secrets は ns manifest の追加 or `managedNamespaceMetadata` で、cilium-secrets は cilium の ns 管理方法を確認して対応
