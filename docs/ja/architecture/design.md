...
|-----------|---------------|---------------|-------------------|
| `kensan-lab` | クラスター基盤、セキュリティ、Argo CD制御構造（App Project、Root Apps） | PE | Istio、Keycloak、Sealed Secrets、Namespaces、RBAC、Argo CD Applications |
| `backstage-app/templates/` | Backstage用のアプリケーション雛形（Kustomizeベース） | PE | Backstageテンプレート定義、ベースKubernetesマニフェスト（kensan-lab内） |
...
5. ADがBackstage経由で新規アプリを作成すると：
   ...
   - BackstageがApplication CRを`kensan-lab/infrastructure/gitops/argocd/applications/`に自動コミット
...
- **IPプール**: `192.168.0.240-192.168.0.249`（`infrastructure/cilium/lb-ippool.yaml`で定義）
...
- **Istio Gateway**: 環境ごと（Prod/Dev）に`infrastructure/istio/`で定義
...
- **自動GitOps**: BackstageがArgo CD Application CRを`kensan-lab`に自動コミット（実装予定）
