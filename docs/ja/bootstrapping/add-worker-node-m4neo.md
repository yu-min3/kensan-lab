...
#### `kubernetes/network/cilium/values.yaml` の変更
...
#### `kubernetes/network/cilium/resources/lb-ippool.yaml` の変更
...
| コンポーネント | ファイル | 追記方法 |
|--------------|---------|---------|
| Prometheus | `kubernetes/observability/prometheus/values.yaml` | `prometheus.prometheusSpec.affinity` に追記 |
| Grafana | `kubernetes/observability/grafana/values.yaml` | トップレベルに `affinity:` ブロック追加 |
| Tempo | `kubernetes/observability/tempo/values.yaml` | 既存の `affinity: {}` を書き換え |
| Loki | `kubernetes/observability/loki/values.yaml` | `singleBinary.affinity` に追記 |
| OTel Collector | `kubernetes/observability/otel-collector/values.yaml` | 既存の `affinity: {}` を書き換え |
...
`kubernetes/auth/keycloak/overlays/prod/affinity-patch.yaml` を作成:
