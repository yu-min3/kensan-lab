...
- Sealed Secretsコントローラーがクラスターにデプロイ済みであること（`kubernetes/sealed-secret/controller.yaml`）。
...
GHCR token は ESO 経由で配布される。Vault path `secret/ghcr/pull-token` に書き込み、`ExternalSecret` (apps/kensan/manifests/{app,data}/, kubernetes/environments/app-prod/, backstage/manifests/) が各 ns に同期する。
...
以下のスクリプトを実行すると、ランダムなパスワードが生成され、暗号化された`SealedSecret`が `kubernetes/prometheus/grafana-sealed-secret.yaml` に保存されます。
...
- `kubernetes/backstage/postgresql-secret.yaml`
- `kubernetes/backstage/backstage-secret.yaml`
