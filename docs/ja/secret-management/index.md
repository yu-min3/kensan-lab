...
GHCR token は ESO 経由で配布される。Vault path `secret/ghcr/pull-token` に書き込み、`ExternalSecret` (apps/kensan/manifests/{app,data}/, infrastructure/environments/app-prod/, backstage/manifests/) が各 ns に同期する。
