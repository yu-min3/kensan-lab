...
- ApplicationSet `environments` 経由で生成されていた env-kensan-dev も自動的に消える (`kubernetes/kensan/kensan-dev/config.json` 削除済)
- NetworkPolicy: `kubernetes/network/network-policy/kensan-dev.yaml` (manifest 側で削除済)
