...
- NetworkPolicy: `kubernetes/network/network-policy/keycloak-dev.yaml` (manifest 側で削除済)
- Vault DB engine config: `kubernetes/secrets/vault-database-engine/platform-values/vault-database/keycloak-dev.yaml` (manifest 側で削除済)
