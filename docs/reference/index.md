...
|---|---|---|
| [Bare-metal bootstrap](../bootstrapping/index.md) | Prerequisites, configuration, and the staged build of the live cluster. The Terraform that establishes Vault's root of trust is written beside the code in `bootstrap/` and published here as [Vault root of trust](../bootstrapping/vault-root-of-trust.md) | **A record, not an installer.** The steps reflect the running system; the full sequence has not been exercised from a blank machine |
