...
| **Vault Transit** | DB に保存する PII の暗号鍵 (前例: 旧 kensan `users.name`) | 手動 rotate (`vault write -f transit/keys/.../rotate`)、新規 encrypt は最新版で実行 | Vault Transit engine (鍵自体が Vault 内、ciphertext のみ DB) |
| **SealedSecret** | Vault 自身の起動依存と、Vault より下のレイヤの cred (cert-manager DNS-01 / Longhorn R2) | 手動 (低頻度) | Git repo |

> **現状 (2026-07)**: kensan-legacy 撤去 (#403/#404) と Keycloak revert ([ADR-019](../adr/019-keycloak-db-credentials-revert-to-static.md)) により、**Vault dynamic / Transit の active instance は現在ゼロ**。レール (chart / mount / ApplicationSet) は稼働済みで、consumer 追加は `platform-values/` に 1 ファイル置くだけ。稼働中の全 secret は Vault static (ESO) か SealedSecret。
...
- Vault 自身 / cert-manager / Longhorn backup など **Vault に依存できない低レイヤ** → SealedSecret（Cilium 内部 PKI はかつてここに居たが、cert-manager `Certificate` 管理へ移行済み — `kubernetes/network/cilium/resources/cilium-ca-certificate.yaml`）
...
**現在 active instance なし**（`vault-database-engine/platform-values/vault-database/` は空）。最後の consumer だった dagster は kensan-legacy 撤去に伴い削除 (#403)、Keycloak は static へ revert ([ADR-019](../adr/019-keycloak-db-credentials-revert-to-static.md))。レール自体（chart + shared + ApplicationSet）は稼働済みで、追加手順は下の Operations と [`vault-database-engine/README.md`](https://github.com/yu-min3/kensan-lab/blob/main/kubernetes/secrets/vault-database-engine/README.md) を参照。
...
| auth-system | oauth2-proxy-secret | client-id / client-secret / cookie-secret | Gateway ext_authz の oauth2-proxy (`platform-auth/istio-gateway/platform`) |
| backstage | backstage-secret | GITHUB_TOKEN | Backstage GitHub 連携 (READ PAT) |
| backstage | postgresql-secret | POSTGRES_USER/PASSWORD/DB | Postgres StatefulSet boot + backstage app（據置 — 下表） |
...
| platform-auth-prod | keycloak-secret | KEYCLOAK_ADMIN_PASSWORD / KC_DB_PASSWORD | Keycloak admin + DB password（`KC_DB_PASSWORD` は `platform-auth/prod/postgresql.POSTGRES_PASSWORD` から同期、[ADR-019](../adr/019-keycloak-db-credentials-revert-to-static.md)） |
| platform-auth-prod | postgresql-secret | POSTGRES_USER/PASSWORD/DB | Keycloak Postgres boot |
| {backstage, app-kensan} | ghcr-pull-secret | `.dockerconfigjson` (GHCR image pull token) | 各 ns の imagePullSecrets。Vault path `secret/ghcr/pull-token` を共有 (app-kensan は app-base chart の externalsecret-ghcr.yaml 経由) |

（旧 `kensan` ns の 6 secret と shared `app-prod` の ghcr-pull-secret は、kensan-legacy 撤去 / per-app ns 移行完了に伴い 2026-07 に消滅）

### GHCR トークンの READ/WRITE 使い分け（統一規約）

GHCR 用 PAT はスコープで 2 本に分ける:

| 種別 | スコープ | 置き場 | 使う場所 |
|---|---|---|---|
| **READ** | `read:packages` (+ repo read) | Vault / local `.env` `READ_GHCR_PAT` | image **pull**（`ghcr-pull-secret` = Vault `secret/ghcr/pull-token`）、Backstage 実行時 `GITHUB_TOKEN`（Vault `secret/backstage/github`、url reader / catalog / TechDocs）、local `make dev`/`run` |
...
**現在 active consumer なし**（`vault-transit-engine/platform-values/vault-transit/` は空）。前例の `users-name` 鍵（旧 kensan user-service の `users.name` カラム暗号化 + HMAC、Stage 6）は kensan-legacy 撤去に伴い consumer instance を削除 (#404)。

レール自体は現役: Vault 側 (mount/policy/auth role) と consumer ns 側 (SA/ConfigMap) は chart 化済 (`vault-transit-engine/chart/` + `platform-values/vault-transit/<consumer>.yaml`)。consumer Deployment は `serviceAccountName` + `envFrom: configMapRef` で参照し、Reloader で rotation 連動。key 作成だけ手動 (`bootstrap/setup-transit-keys.sh` — VCO 限界、上の「VCO カバレッジと例外」参照)。rotate は `vault write -f transit/keys/<name>/rotate` → アプリ側で `transit/rewrap`。

設計詳細・consumer 追加手順: [`kubernetes/secrets/vault-transit-engine/README.md`](https://github.com/yu-min3/kensan-lab/tree/main/kubernetes/secrets/vault-transit-engine)
...
（Cilium / Hubble 内部 PKI はかつて SealedSecret だったが、chart 側 Secret との ownerRef 衝突で takeover 不可のため cert-manager `Certificate` 管理へ移行 — `kubernetes/network/cilium/resources/cilium-ca-certificate.yaml` のヘッダコメント参照）

### 据置 (static のまま維持)

| アプリ | 理由 | 方針 |
|--------|------|----------|
| keycloak | DB credential rotation による Pod restart が SSO session を破壊する。IdP は Vault / SSO の中核で循環依存も大きい（dynamic 化を実施→revert した経緯: [ADR-019](../adr/019-keycloak-db-credentials-revert-to-static.md)） | Vault static (ESO) の `keycloak-secret` に固定 |
| backstage | 長寿命 service で 12h restart の価値が薄い。plugin DB / schema owner 問題もある | Vault static (ESO) の `postgresql-secret` に固定 |

（旧 kensan 系の據置 polaris / kensan-app は kensan-legacy 撤去 ([ADR-017](../adr/017-kensan-legacy-removal.md)) で対象ごと消滅）
