...
| kensan | postgres-polaris-cred | polaris (Iceberg catalog metastore) | ✅ 使用中 |
...
| backstage | postgres-backstage-cred | backstage | ✅ 使用中 (2026-06-07 cutover。plugin DB x11 は chart の `grantRootOwnerMembership` で解決) |
...
| backstage | postgresql-secret | POSTGRES_USER/PASSWORD/DB | Postgres StatefulSet boot + VCO root cred |
...
| ~~backstage~~ | ~~12 DB 分散接続 vs dbName 単位 GRANT~~ → **2026-06-07 解消**: chart の `grantRootOwnerMembership` (group role 戦略) + initdb ConfigMap (DR 用 plugin DB pre-create) で cutover 完了 | — |
