...
2. **Every layer of health monitoring exists because something real slipped past the previous one.** An 8-day silent node freeze motivated the off-cluster dead-man's switch (layer ③); an auth-path breakage that hid for 3 weeks motivated probing the *authenticated* HTTPS endpoints, not just ping (layer ②). The phased design is [cluster-health-monitoring.md](https://github.com/yu-min3/kensan-lab/blob/main/docs/architecture/cluster-health-monitoring.md); the incidents are in [docs/incidents](https://github.com/yu-min3/kensan-lab/tree/main/docs/incidents).
